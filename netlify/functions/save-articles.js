...
exports.handler = async (event) => {
  ...
  try {
    const res = await httpsPatch(
      ...
      headers,
      { files: { 'articles-data.json': { content: JSON.stringify(body, null, 2) } } }
    );
    if (res.status !== 200) return { statusCode: res.status, body: res.body };

    // Disparar rebuild de Netlify si está configurado
    const buildHook = process.env.NETLIFY_BUILD_HOOK || '';
    if (buildHook) {
      try {
        const hookUrl = new URL(buildHook);
        await new Promise((resolve) => {
          const req = https.request({ hostname: hookUrl.hostname, path: hookUrl.pathname + hookUrl.search, method: 'POST', headers: { 'Content-Length': 0 } }, (r) => { r.resume(); r.on('end', resolve); });
          req.on('error', resolve);
          req.end();
        });
      } catch (_) { /* no bloquear si falla el hook */ }
    }
